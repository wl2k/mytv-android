package top.wl2k.mytv.ui.screens.leanback.classicpanel.components
    ...
    if (epg != null && epg.programmes.isNotEmpty()) {
        ...
        val programmesListState = remember(programmes) {
            LazyListState(
                max(
                    0,
                    programmes.indexOfFirst { it.isLive() } - 2))
        }
        val daysListState = remember(programmesGroup) {
            LazyListState(
                max(
                    0,
                    programmesGroup.keys.indexOf(currentDay) - 2
                )
            )
        }
